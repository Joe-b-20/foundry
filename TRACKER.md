...
## 2026-06-10 — REPO REORG for publication (rediscovery-engine)
What I tried: turn the working directory into a publishable repo per Joe. Layout: src/ (all .py, FLAT on purpose — sibling imports preserved: expJ<-expG, gpu_exp1b<-gpu_exp1, loop<-oe), scripts/ (orchestration .sh + runpod_plan, cd/paths fixed to run from root), consolidation/ + writeup/ + runs/ + runs_pod/ unchanged, run.sh stays at root (invocation: bash run.sh src/file.py). NEW: public README.md with Mermaid diagrams (project arc, wall taxonomy with status colors, the audit loop, the identity pipeline), LICENSE (MIT), src/README + _private/README. PRIVACY/SIZE: _private/ (gitignored, organized: bigdata/ for >50MB artifacts with pointer files left in place [pcf_main stage1 npz 231MB > GitHub hard limit], infra/ for pod credentials [PLAYBOOK scrubbed, original kept], scratch/). HISTORY: the 231MB npz is in main history -> a push of main would be rejected; created an ORPHAN BRANCH public (single clean release commit, no _private, no oversized blobs, 124MB content) — push that to GitHub; local main keeps the full audit-trail history private.
What happened: main commit 42f4194 (reorg) + public branch 19cd273 (release snapshot). Post-move smokes pass: fn_telescope --selftest (exercises src sibling imports + controls) and expCC_ladder both clean; no tracked file >=50MB on public; quoting-trap hit once more during scrub (backticks in printf — used Write tool instead, per PLAYBOOK trap list).
...
## 2026-06-10 — FOUNDRY v2 (controls fixed): clean control-gated run — reference-hardening absorbs all finds; honest null at small budget; mutant-control artifact identified
What I tried: closed run-1 gaps. (1) DIAGNOSED+FIXED the zeta3 control failure: the v1 verify path rebuilt the constant battery at the wrong precision mid-check (and make_battery mutated global dps) -> verified=False on real hits. v2: clean _hp_const lookup at the verify dps + battery builder restores dps. (2) Per-universe INJECTED CONTROLS with explicit C+/C-! gating (zeta3-home carries the RM A=[1,5,9,6], catalan-home the kappa=0 form, zeta2-home Apery zeta2). (3) Reference DB extended to the FULL published Table-7 (i,j,mu) generators of arXiv:2210.15669 (~40 members; zero-valued j=0 members crashed PSLQ -> guarded). (4) Drop-reason logging (unv/multi/rational) so nothing fails silently. Run: 20 gens, pop 8, pod.
What happened (runs_pod/phase2/foundry2/foundry_log.json): ALL 18 home-universe control checks C+ across the run — gen-0 zeta3-home now shows near=4 ver=2 (the sweep itself recovers the RM pair and reference-matches it; v1 mystery fully resolved). NOVEL-FLAGGED: 0 — the Table-7 hardening absorbs exactly the family members run 1 mis-flagged as novel (g5/g6-class hits now match references). No pockets, no latent constants. 53 C-! flags are ALL on MUTANT universes — a design artifact: mutants inherit the parent control_A, which cannot exist under a mutated B-form; v3 fix = drop inherited controls on B/index mutations (mutant universes are currently control-less, their nulls weak).
What I learned: the foundry is now a CLEAN instrument: control-gated, fully self-deduplicating, drop-transparent — and at small budgets it correctly classifies everything it reaches as known, which is the machine WORKING (run 1 + literature taught it the family; run 2 stopped re-celebrating it). The remaining path to a genuine pocket is budget+breadth (bigger universes, more B-families, longer runs) on a designed paid campaign — the instrument is ready. Known v3 items: mutant control inheritance; cross-universe pocket scoring; prime-indexed control identity (still an open mini-problem).
Status: WORKS (v2 instrument validated: 18/18 home controls C+, reference-subtraction complete, honest null). n=1 run, 20 gens.
Files: src/foundry.py (v2), runs_pod/phase2/foundry2/, runs_pod/phase2/FOUNDRY2.out
