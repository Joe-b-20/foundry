...
run pcf_stage1 $PY -u src/gpu_pcf_hunt.py --stage1 --crange 6 --terms 90 --out runs/pcf_main
run pcf_stage2 $PY -u src/gpu_pcf_hunt.py --stage2 --procs 60 --dps 250 --limit 400000 --out runs/pcf_main
...
for seed in 1 2; do
  for mit in 1 4 16; do
    run loop_m${mit}_s${seed} $PY -u src/gpu_avida_loop.py --maxit $mit --N 6144 --P 24 --gens 600 --seed $seed --out runs/loop_m${mit}_s${seed}
  done
done
